...
export function runDoctor(repoRoot: string): { findings: DoctorFinding[]; exitCode: number } {
  ...
  const prismaClientPaths = [
    join(repoRoot, "packages/db/prisma/generated/client.ts"),
    join(repoRoot, "packages/db/prisma/generated/client"),
  ];
  if (!prismaClientPaths.some((clientPath) => existsSync(clientPath))) {
    findings.push({
      severity: "info",
      category: "database",
